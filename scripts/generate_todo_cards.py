...
from scholaraio.todo_compass import resolve_external_metadata
...
PROFESSOR_A_SYSTEM_PROMPT = """你是第一位独立评审教授，长期研究 AI、机器人学习、人形机器人控制与具身智能。

你不能看到另一位教授的意见，也不能假设后续会有人替你补全证据。请独立完成判断：
- 先抓“意义性原理”：这篇论文真正改变了什么问题表述、机制假设或研究路线；
- 再做批判性判断：哪些结论有实验支撑，哪些只是作者主张或工程经验；
- 对机器人/AI 读者保持有用，不写空泛赞美。

""" + CARD_SYSTEM_PROMPT

PROFESSOR_B_SYSTEM_PROMPT = """你是第二位独立评审教授，长期研究 AI、机器人学习、人形机器人控制与具身智能。

你不能看到另一位教授的意见，也不能假设后续会有人替你补全证据。请独立完成判断：
- 先检查方法机制是否真的成立，尤其是 baseline、公平比较、消融和部署边界；
- 再判断它对 AI+机器人研究的真实增量，避免把作者叙事直接当事实；
- 对数字、benchmark、理论证明和失败条件保持保守。

""" + CARD_SYSTEM_PROMPT

METADATA_PROFESSOR_A_SYSTEM_PROMPT = """你是第一位独立评审教授，长期研究 AI、机器人学习、人形机器人控制与具身智能。

你只能基于给定元数据、摘要和公开补充线索判断，不能看到另一位教授的意见。请特别关注这篇论文的意义性原理：它可能改变了什么问题表述、机制假设或研究路线。证据不足时必须保守。

""" + CARD_METADATA_SYSTEM_PROMPT

METADATA_PROFESSOR_B_SYSTEM_PROMPT = """你是第二位独立评审教授，长期研究 AI、机器人学习、人形机器人控制与具身智能。

你只能基于给定元数据、摘要和公开补充线索判断，不能看到另一位教授的意见。请特别关注批判性校验：benchmark、数值、消融、理论证明和真实机器人部署如果没有明确证据，必须写清楚信息不足。

""" + CARD_METADATA_SYSTEM_PROMPT

CARD_SYNTHESIS_SYSTEM_PROMPT = """你是 Todo 阅读卡片的最终综合编辑。你会拿到两位互不沟通的 AI+机器人领域教授的独立 JSON 评审。

你的任务不是简单拼接，而是综合成一张最终 Todo card：
1. 保留两位教授都认为重要、且机制清楚的内容；
2. 对冲突信息采取更保守、更可由原文/公开证据支持的说法；
3. 删除重复表达、空泛评价和没有阅读价值的套话；
4. 不能编造两个评审都没有支持的 benchmark 数字、理论证明、开源状态或部署结论；
5. 输出必须以中文为主，术语可保留英文；
6. 最终只输出下面 schema 的严格 JSON，不要 markdown，不要代码块，不要额外解释。
...
DUAL_PROFESSOR_PROTOCOL = "dual_professor_synthesis_v1"
DUAL_PROFESSOR_ROLES = (
    "professor_a_meaning_principles",
    "professor_b_critical_validation",
)
...
def _format_external_context(meta: dict[str, Any], paper_dir: Path | None = None) -> str:
    if paper_dir is None:
        ...
        return "\n".join(lines) if lines else "公开补充线索不足。"

    try:
        external = resolve_external_metadata(meta, paper_dir)
    except Exception as exc:
        return f"公开补充线索获取失败，生成时只使用本地证据。错误: {_truncate(str(exc), 120)}"

    lines = []
    if external.get("paper_url"):
        lines.append(f"- 论文/公开页面: {external['paper_url']}")
    if external.get("venue_name"):
        lines.append(f"- 公开 venue: {external['venue_name']}")
    if external.get("impact_text"):
        lines.append(f"- 公开影响力线索: {external['impact_text']}")
    if external.get("openalex_url"):
        lines.append(f"- OpenAlex: {external['openalex_url']}")
    if external.get("crossref_url"):
        lines.append(f"- Crossref: {external['crossref_url']}")
    if external.get("arxiv_id"):
        lines.append(f"- arXiv ID: {external['arxiv_id']}")
    return "\n".join(lines) if lines else "公开补充线索不足。"


def _has_external_context_signal(context: str) -> bool:
    return bool(context.strip()) and "公开补充线索不足" not in context and "公开补充线索获取失败" not in context
...
def _card_needs_quality_refresh(card: dict[str, Any]) -> bool:
    if str(card.get("generation_protocol") or "").strip() != DUAL_PROFESSOR_PROTOCOL:
        return True
    if int(card.get("reviewer_count") or 0) != 2:
        return True
...
def _run_model(
    ...
    *,
    model: str,
    timeout: int,
    system: str = CARD_SYSTEM_PROMPT,
    purpose: str = "todo.cards",
) -> dict[str, Any]:
...
def _dual_professor_metadata(*, model: str) -> dict[str, Any]:
    return {
...
def _build_synthesis_prompt(
    *,
    source_context: str,
    professor_a: dict[str, Any],
    professor_b: dict[str, Any],
) -> str:
    return (
        "请根据下面的论文简要信息与两位独立教授评审，生成最终 Todo 阅读卡片。\n\n"
        f"论文简要信息:\n{source_context.strip()}\n\n"
        "教授 A 独立评审 JSON:\n"
        f"{json.dumps(professor_a, ensure_ascii=False, indent=2)}\n\n"
        "教授 B 独立评审 JSON:\n"
        f"{json.dumps(professor_b, ensure_ascii=False, indent=2)}\n"
    )


def _run_dual_professor_model(
    prompt: str,
    source_context: str,
    cfg,
    *,
    ...
    if metadata_only:
        professor_a_system = METADATA_PROFESSOR_A_SYSTEM_PROMPT
        professor_b_system = METADATA_PROFESSOR_B_SYSTEM_PROMPT
    else:
        professor_a_system = PROFESSOR_A_SYSTEM_PROMPT
        professor_b_system = PROFESSOR_B_SYSTEM_PROMPT

    with ThreadPoolExecutor(max_workers=2) as executor:
        professor_a_future = executor.submit(
            _run_model,
            prompt,
            cfg,
            model=model,
            timeout=timeout,
            system=professor_a_system,
            purpose="todo.cards.professor_a",
        )
        professor_b_future = executor.submit(
            _run_model,
            prompt,
            cfg,
            model=model,
            timeout=timeout,
            system=professor_b_system,
            purpose="todo.cards.professor_b",
        )
        professor_a = professor_a_future.result()
        professor_b = professor_b_future.result()
    final = _run_model(
        _build_synthesis_prompt(
            source_context=source_context,
            professor_a=professor_a,
            professor_b=professor_b,
        ),
        cfg,
        model=model,
        timeout=timeout,
        system=CARD_SYNTHESIS_SYSTEM_PROMPT,
        purpose="todo.cards.synthesis",
    )
    final.update(_dual_professor_metadata(model=model))
    return final
...
def _generate_one(
    item: MatchedTodoPaper,
    cfg,
    *,
    model: str,
    timeout: int,
    max_l4_chars: int = DEFAULT_MAX_L4_CHARS,
) -> dict[str, Any]:
    if not item.dir_name:
        external_context = _format_external_context(
            {
                "title": item.title,
                "doi": item.doi or item.zotero_doi,
                "journal": item.journal,
        ...
        has_external_signal = _has_external_context_signal(external_context)
        metadata_prompt = (
        ...
        source_context = (
            ...
            f"期刊/会议: {item.journal}\n"
            f"DOI: {item.doi or item.zotero_doi}\n"
            f"摘要: {_truncate(_strip_html(item.abstract or '') or '摘要缺失', 1600)}\n"
            f"公开补充线索:\n{external_context}"
        )
        try:
            generated = _run_dual_professor_model(
                metadata_prompt,
                source_context,
                cfg,
                model=model,
            ...
            generation_mode = "dual_agent_synthesis"
        except Exception as exc:
        ...
        return {
            ...
            "collection_key": TODO_COLLECTION_KEY,
            "collection_index": item.collection_index,
            "analysis_source": "metadata+web" if has_external_signal else "metadata",
            "source_confidence": "medium" if has_external_signal else "limited",
    ...
    content = _get_paper_content(paper_dir, max_l4_chars=max_l4_chars)
    external_context = _format_external_context(content["l1"], paper_dir)
    has_external_signal = _has_external_context_signal(external_context)
    prompt = (
        "以下是论文输入。正文可能是截断摘录，请优先依据标题、摘要、结论、实验片段和公开补充线索进行谨慎泛读分析。\n\n"
        f"标题: {content['l1'].get('title', '')}\n"
        f"作者: {', '.join(content['l1'].get('authors', []))}\n"
        f"年份: {content['l1'].get('year', '')}\n"
        f"期刊/会议: {content['l1'].get('journal', '')}\n"
    ...
    source_context = (
        ...
        f"期刊/会议: {content['l1'].get('journal', '')}\n"
        f"DOI: {content['l1'].get('doi', '')}\n"
        f"摘要: {_truncate(content['l2'], 1600)}\n"
        f"结论: {_truncate(content['l3'], 1600)}\n"
        f"公开补充线索:\n{external_context}"
    )

    try:
        generated = _run_dual_professor_model(prompt, source_context, cfg, model=model, timeout=timeout)
        generation_mode = "dual_agent_synthesis"
    except Exception as exc:
    ...
    return {
        ...
        "collection_key": TODO_COLLECTION_KEY,
        "collection_index": item.collection_index,
        "analysis_source": "fulltext+web" if has_external_signal else "fulltext",
...
def main() -> None:
    ...
    parser.add_argument(
        "--max-l4-chars",
        type=int,
        default=DEFAULT_MAX_L4_CHARS,
        help="Maximum paper.md characters sent to each professor reviewer.",
    )
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as executor:
        future_map = {
            executor.submit(
                ...
                max_l4_chars=max(1000, args.max_l4_chars),
            ): item
            for item in pending
        }
        completed = 0
