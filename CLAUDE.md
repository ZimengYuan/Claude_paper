...
Todo 卡片必须使用“双教授协议”。先生成两份彼此独立的分析：两位评审都设定为 AI + 机器人领域资深教授，都必须具备意义性原理判断和批判思维，并且互相看不到对方输出。第一位更强调论文背后的研究意义、机制假设和路线变化；第二位更强调实验有效性、baseline、公平比较、局限和部署边界。最终卡片必须综合两份独立评审，而不是直接拼接。发布的卡片 schema 保持为最终总结为主，只保留轻量 provenance 字段，例如 `generation_protocol=dual_professor_synthesis_v1`、`reviewer_count=2` 和 reviewer role names。
